...
/// * self_tid - Type written as the projection self type, such as `T`
/// * assoc_type - Definition of the associated type being projected, such as `Trait::Assoc`
/// * trait_tid - Explicit trait path type, such as `Trait` in `<T as Trait>::Assoc`
///
/// # Examples
///
/// * Code - `<T as Trait>::Assoc`
/// * Input - `projection = ty0`, `self_tid = ty1`, `assoc_type = def2`, `trait_tid = ty3`
/// * Output - `explicit_projection_obligation(ty0, ty1, def2, ty3).`
/// * Code - `<T>::Assoc`
/// * Input - `projection = ty0`, `self_tid = ty1`, `assoc_type = def2`, `trait_tid = None`
...
pub(in crate::logic) fn projection_obligation_clause<'cx>(
    ...
    let projection = type_id(ccx, obligation.projection_tid);
    let self_tid = type_id(ccx, self_tid);
    ...
    if let Some(trait_tid) = obligation.trait_tid {
        Clause {
            head: explicit_projection_obligation(
                ...
                self_tid,
                assoc_type,
                type_id(ccx, trait_tid),
            ),
            body: None,
        }
    } else {
        Clause {
            head: projection_obligation(ccx, projection, self_tid, assoc_type),
            body: None,
        }
    }
}
...
/// * trait_tid - Trait required by the bound, such as `Trait`
///
/// # Examples
///
/// * Code - `T: Trait`
/// * Input - `subject = ty0`, `trait_tid = ty1`
...
pub(in crate::logic) fn trait_bound_clause<'cx>(
    ...
    Clause {
        head: trait_bound(
            ccx,
            type_id(ccx, bound.subject_tid),
            type_id(ccx, bound.trait_tid),
        ),
        body: None,
    }
}

/// * impl_self_tid - Implementing self type in `impl Trait for Self`
/// * trait_tid - Implemented trait type in `impl Trait for Self`
/// * assoc_type - Associated type definition assigned by the impl item
/// * value_tid - Type assigned by the impl item
///
/// # Examples
///
/// * Code - `impl Iterator for Vec { type Item = u32; }`
/// * Input - `impl_self_tid = ty0`, `trait_tid = ty1`, `assoc_type = def2`, `value_tid = ty3`
...
pub(in crate::logic) fn impl_assoc_type_clause<'cx>(
    ...
    Clause {
        head: impl_assoc_type(
            ccx,
            type_id(ccx, fact.impl_self_tid),
            type_id(ccx, fact.trait_tid),
            def_id(ccx, fact.assoc_type),
            type_id(ccx, fact.value_tid),
        ),
        body: None,
    }
}

/// * projection_self_tid - Self type from the projection, such as `Vec<u32>`
/// * impl_self_tid - Self type from the impl header, such as `Vec<T>`
///
/// # Examples
///
/// * Code - `<Vec<u32> as Iterator>::Item` matched against `impl<T> Iterator for Vec<T>`
/// * Input - `projection_self_tid = ty0`, `impl_self_tid = ty1`
...
pub(in crate::logic) fn impl_self_match_clause<'cx>(
    ...
    Clause {
        head: impl_self_match(
            ccx,
            type_id(ccx, match_.projection_self_tid),
            type_id(ccx, match_.impl_self_tid),
        ),
        body: None,
    }
}
...
/// * projection_self_tid - Self type from the projection, such as `Vec<u32>`
/// * impl_self_tid - Self type from the impl header, such as `Vec<T>`
/// * generic_tid - Generic type occurrence from the impl self type, such as `T`
/// * arg_tid - Type argument matched for the generic, such as `u32`
///
/// # Examples
///
/// * Code - `<Vec<u32> as Iterator>::Item` matched against `impl<T> Iterator for Vec<T>`
/// * Input - `projection_self_tid = ty0`, `impl_self_tid = ty1`, `generic_tid = ty2`, `arg_tid = ty3`
...
pub(in crate::logic) fn type_binding_clause<'cx>(
    ...
    Clause {
        head: type_binding(
            ccx,
            type_id(ccx, binding.projection_self_tid),
            type_id(ccx, binding.impl_self_tid),
            type_id(ccx, binding.generic_tid),
            type_id(ccx, binding.arg_tid),
        ),
        body: None,
    }
}

/// * projection_self_tid - Self type from the projection that requested the substitution
/// * impl_self_tid - Self type from the impl header whose value type is substituted
/// * value_tid - Type before substitution, such as `T`
/// * generic_tid - Generic type occurrence being substituted, such as `T`
/// * arg_tid - Type argument used for the generic, such as `u32`
/// * substituted_tid - Type after substitution, such as `u32`
///
/// # Examples
///
/// * Code - `type Item = T` with `Vec<T>` matched against `Vec<u32>`
/// * Input - `projection_self_tid = ty0`, `impl_self_tid = ty1`, `value_tid = ty2`,
///   `generic_tid = ty2`, `arg_tid = ty3`, `substituted_tid = ty3`
...
pub(in crate::logic) fn type_substitution_clause<'cx>(
    ...
    Clause {
        head: type_substitution(
            ccx,
            type_id(ccx, substitution.projection_self_tid),
            type_id(ccx, substitution.impl_self_tid),
            type_id(ccx, substitution.value_tid),
            type_id(ccx, substitution.generic_tid),
            type_id(ccx, substitution.arg_tid),
            type_id(ccx, substitution.substituted_tid),
        ),
        body: None,
    }
}
...
/// * trait_tid - Candidate trait type that may provide the associated type
///
/// # Examples
///
...
pub(in crate::logic) fn projection_candidate_clause<'cx>(
    ...
    Clause {
        head: projection_candidate(
            ccx,
            type_id(ccx, candidate.projection_tid),
            type_id(ccx, candidate.self_tid),
            def_id(ccx, candidate.assoc_type),
            type_id(ccx, candidate.trait_tid),
        ),
        body: None,
    }
}
...
/// * trait_tid - Candidate trait type that provides the associated type
///
/// # Examples
///
...
pub(in crate::logic) fn projection_match_clause<'cx>(
    ...
    Clause {
        head: projection_match(
            ccx,
            type_id(ccx, match_.projection_tid),
            type_id(ccx, match_.self_tid),
            def_id(ccx, match_.assoc_type),
            type_id(ccx, match_.trait_tid),
        ),
        body: None,
    }
}

/// * trait_tid - Trait type that owns the member
...
/// * Input - `trait_tid = ty0`, `requested_assoc_type = def1`, `member_assoc_type = def2`
...
pub(in crate::logic) fn projection_candidate_query<'cx>(
    ...
    Expr::Term(projection_candidate(
        ccx,
        type_id(ccx, projection_tid),
        type_id(ccx, self_tid),
...
/// * self_tid - Type written as the projection self type, such as `Vec`
/// * assoc_type - Associated type member used for normalization, such as `Iterator::Item`
/// * trait_tid - Trait type that provides the associated type, such as `Iterator`
/// * value_tid - Type assigned by the matching impl item, such as `u32`
///
/// # Examples
///
/// * Code - can `<Vec as Iterator>::Item` normalize to `u32`?
/// * Input - `projection = ty0`, `self_tid = ty1`, `assoc_type = def2`, `trait_tid = ty3`, `value_tid = ty4`
...
fn projection_candidate<'cx>(
    ccx: &'cx CommonCx,
    projection_tid: LogicTerm<'cx>,
    self_tid: LogicTerm<'cx>,
    assoc_type: LogicTerm<'cx>,
    trait_tid: LogicTerm<'cx>,
) -> LogicTerm<'cx> {
    term(
        ccx,
        PRED_PROJECTION_CANDIDATE,
        vec![projection_tid, self_tid, assoc_type, trait_tid],
    )
}
...
fn projection_normalizes_to<'cx>(
    ...
    assoc_type: LogicTerm<'cx>,
    trait_tid: LogicTerm<'cx>,
    value_tid: LogicTerm<'cx>,
) -> LogicTerm<'cx> {
    term(
        ccx,
        PRED_PROJECTION_NORMALIZES_TO,
        vec![projection_tid, self_tid, assoc_type, trait_tid, value_tid],
    )
}
...
/// * self_tid - Type written as the projection self type, such as `T`
/// * assoc_type - Definition of the associated type being projected, such as `Trait::Assoc`
/// * trait_tid - Explicit trait path type, such as `Trait`
///
/// # Examples
///
/// * Code - `<T as Trait>::Assoc`
...
fn impl_assoc_type<'cx>(
    ...
    term(
        ...
        vec![impl_self_tid, trait_tid, assoc_type, value_tid],
    )
}
...
fn impl_self_match<'cx>(
    ccx: &'cx CommonCx,
    projection_self_tid: LogicTerm<'cx>,
    impl_self_tid: LogicTerm<'cx>,
) -> LogicTerm<'cx> {
    term(
        ccx,
        PRED_IMPL_SELF_MATCH,
        vec![projection_self_tid, impl_self_tid],
    )
}
...
fn type_binding<'cx>(
    ...
    projection_self_tid: LogicTerm<'cx>,
    impl_self_tid: LogicTerm<'cx>,
    generic_tid: LogicTerm<'cx>,
    arg_tid: LogicTerm<'cx>,
) -> LogicTerm<'cx> {
    term(
        ccx,
        PRED_TYPE_BINDING,
        vec![projection_self_tid, impl_self_tid, generic_tid, arg_tid],
    )
}
...
///
/// * Code - `<T>::Assoc`
/// * Input - `projection = ty0`, `self_tid = ty1`, `assoc_type = def2`
/// * Output - `projection_obligation(ty0, ty1, def2)`
fn projection_obligation<'cx>(
    ...
    term(
        ...
        vec![projection_tid, self_tid, assoc_type],
    )
}
...
fn trait_bound<'cx>(
    ...
    term(ccx, PRED_TRAIT_BOUND, vec![subject, trait_tid])
}
...
fn trait_member<'cx>(
    ...
    term(
        ccx,
        PRED_TRAIT_MEMBER,
        vec![trait_tid, requested_assoc_type, member_assoc_type],
    )
}
