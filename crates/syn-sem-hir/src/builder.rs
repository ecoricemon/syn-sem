...
impl<'cx> HirArenaBuilder<'cx> {
    ...
    fn fill_type(&mut self, ty_id: TypeId, ty: Type<'cx>) {
        assert_eq!(ty_id, ty.id);
        self.types.fill(ty_id, ty);
    }
}
...
mod tests {
    ...
    fn represents_type_paths_and_nested_generic_arguments() {
        ...
        let GenericArg::Type(iter_ty_id) = last.args[1] else {
            panic!("expected type argument");
        };
        let TypeKind::Path(iter_path) = &model[iter_ty_id].kind else {
